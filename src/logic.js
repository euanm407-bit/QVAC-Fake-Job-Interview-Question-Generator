// QVAC Fake Job Interview Question Generator — core logic.
// completion() writes the result from a single user input field.
// The weirdness meter is a deterministic score from keyword density
...
const FALLBACK = (v) => `If you were a ${v} stuck on a desert island, what's the first process you'd optimize?`;
...
export async function generate(modelId, input) {
  const run = completion({
    ...
    history: [
      {
        role: "system",
        content: ((v) => `Write one short, oddly specific, slightly absurd job interview question (1 sentence) for a candidate applying to be a ${v}. Reply with ONLY the question, no preamble.`)(input),
      },
  ...
  return { result, weirdness: scoreWeirdness(result) };
}

const KEYWORDS = ["imagine", "if you were", "describe", "hypothetical", "scenario", "unusual", "creative"];

function scoreWeirdness(text) {
  const lower = text.toLowerCase();
  let score = 45;
