#!/usr/bin/env node
// QVAC Fake Job Interview Question Generator — GUI mode.
...
const PORT = process.env.PORT ? Number(process.env.PORT) : 29386;
...
async function main() {
  ...
  const server = http.createServer(async (req, res) => {
    ...
    if (req.method === "POST" && req.url === "/api/interviewquestion") {
      try {
        ...
        const result = await generate(modelId, job.trim());
  ...
  server.listen(PORT, () => {
    console.log(`▸ QVAC Fake Job Interview Question Generator GUI ready at http://localhost:${PORT}`);
  });
